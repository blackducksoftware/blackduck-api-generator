...
public class FileFinder {
    ...
    private List<File> findFilesRecursive(final File sourceDirectory, final int currentDepth, final int maxDepth, final StringBuilder maxDepthHitMsgPattern, final Boolean recurseIntoDirectoryMatch, final String... filenamePatterns) {
    ...
    public List<File> findDirectories(final File sourceDirectory, final String directoryPattern) {
        final Set<File> files = new HashSet<>();
        if (!sourceDirectory.isDirectory()) {
            return new ArrayList<>(files);
        }
        for (final File file : sourceDirectory.listFiles()) {
            if (file.isDirectory() && FilenameUtils.wildcardMatchOnSystem(file.getName(), directoryPattern)) {
                files.addAll(findDirectories(file, directoryPattern));
            }
        }
        return new ArrayList<>(files);
    }
